    ...
    <article className="dsh-wallpaper-card" data-active={props.active}>
      ...
      <div className="dsh-wallpaper-card-body">
        ...
        <div className="dsh-wallpaper-card-actions">
          <button className="dsh-wallpaper-button" data-primary={!props.active} disabled={props.busy || props.active} onClick={() => { void props.controller.activate(props.record.id) }}>{props.active ? '使用中' : '启用'}</button>
          <button className="dsh-wallpaper-button" data-danger="true" disabled={props.busy} onClick={() => { void remove() }}>删除</button>
        </div>
      </div>
    </article>
  )
  ...
  return (
    <section className="dsh-wallpaper-page" aria-labelledby="dsh-wallpaper-title">
      ...
      <div className="dsh-wallpaper-source-grid">
        <div className="dsh-wallpaper-source"><h3>上传本地文件</h3><p className="dsh-wallpaper-muted">JPG、PNG、WebP、GIF、MP4 或 WebM，单文件最大 100 MB。</p><label className="dsh-wallpaper-button" data-primary="true">选择文件<input hidden type="file" accept={ACCEPTED_FILES} disabled={busy} onChange={upload} /></label></div>
        <form className="dsh-wallpaper-source" onSubmit={addUrl}><h3>添加网络 URL</h3><div className="dsh-wallpaper-source-row"><label className="dsh-wallpaper-field">名称（可选）<input className="dsh-wallpaper-input" value={urlName} onChange={event => { setUrlName(event.target.value) }} /></label><label className="dsh-wallpaper-field">格式<select className="dsh-wallpaper-select" value={urlType} onChange={event => { setUrlType(event.target.value as WallpaperMediaType) }}><option value="image/jpeg">JPG</option><option value="image/png">PNG</option><option value="image/webp">WebP</option><option value="image/gif">GIF</option><option value="video/mp4">MP4</option><option value="video/webm">WebM</option></select></label></div><div className="dsh-wallpaper-source-row"><label className="dsh-wallpaper-field">HTTP(S) 地址<input className="dsh-wallpaper-input" type="url" required placeholder="https://…" value={url} onChange={event => { setUrl(event.target.value) }} /></label><button className="dsh-wallpaper-button" data-primary="true" disabled={busy || url.trim() === ''}>添加</button></div></form>
      </div>
